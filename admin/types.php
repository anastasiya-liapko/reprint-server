...
$next_order['default_price']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-file"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Типы бумаги".' </h2>
...
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=types' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span></td>", $item, "Название"):"<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=types' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span></td>")."
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=types' data-pk='{$item['id']}' data-name='default_price'>".htmlspecialchars($item['default_price'])."</span></td>", $item, "Цена по умолчанию"):"<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=types' data-pk='{$item['id']}' data-name='default_price'>".htmlspecialchars($item['default_price'])."</span></td>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM types WHERE id=?",[$id]);
			$item = $item[0];
		}
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Типы бумаги".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `types` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
...
$default_price = $_REQUEST['default_price'];
		...
		$sql = "INSERT INTO types (`name`, `default_price`) VALUES (?, ?)";
...
$set[] = is_null($_REQUEST['default_price'])?"`default_price`=NULL":"`default_price`='".addslashes($_REQUEST['default_price'])."'";
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE types SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM types WHERE id=?", [$id]);
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM types main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM types main_table) temp $srch $filter $where $order";
			if(function_exists("processSelectQuery"))
			{
				$sql = processSelectQuery($sql);
			}
	...
	echo masterRender("Типы бумаги", $content, 0);
