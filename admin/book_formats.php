<?php
	...
	$actions[''] = function()
	{
   		...
   		$format_id_values = json_encode(q("SELECT name as text, id as value FROM formats", []));
				  $format_id_values_text = "";
			foreach(json_decode($format_id_values, true) as $opt)
			{
			  $format_id_values_text.="<option value=\"{$opt['value']}\">{$opt['text']}</option>";
			}
...
$next_order['is_default']='asc';
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Форматы для {$_REQUEST["book_name"]}".' </h2>
		...
		$table='
			...
			<th>
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=format_id&sort_order='. ($next_order['format_id']) .'\' class=\'sort\' column=\'format_id\' sort_order=\''.$sort_order['format_id'].'\'>Наименование'. $sort_icon['format_id'].'</a>
			</th>
...
".(function_exists("processTD")?processTD("<td><span class='editable ' data-inp='select' data-type='select' data-source='".htmlspecialchars($format_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=book_formats' data-pk='{$item['id']}' data-name='format_id'>".select_mapping($format_id_values, $item['format_id'])."</span></td>", $item, "Наименование"):"<td><span class='editable ' data-inp='select' data-type='select' data-source='".htmlspecialchars($format_id_values, ENT_QUOTES, 'UTF-8')."' data-url='engine/ajax.php?action=editable&table=book_formats' data-pk='{$item['id']}' data-name='format_id'>".select_mapping($format_id_values, $item['format_id'])."</span></td>")."
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=book_formats' data-pk='{$item['id']}' data-name='price'>".htmlspecialchars($item['price'])."</span></td>", $item, "Цена"):"<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=book_formats' data-pk='{$item['id']}' data-name='price'>".htmlspecialchars($item['price'])."</span></td>")."
".(function_exists("processTD")?processTD("<td><span class=' '>".renderRadioGroup("is_default", $is_default_values, "book_formats", $item['id'], $item['is_default'])."</td>", $item, "Оригинал"):"<td><span class=' '>".renderRadioGroup("is_default", $is_default_values, "book_formats", $item['id'], $item['is_default'])."</td>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM book_formats WHERE id=?",[$id]);
			$item = $item[0];
		}

		
			$format_id_options = q("SELECT name as text, id as value FROM formats",[]);
			$format_id_options_html = "";
			foreach($format_id_options as $o)
			{
				$format_id_options_html .= "<option value=\"{$o['value']}\" ".($o["value"]==$item["format_id"]?"selected":"").">{$o['text']}</option>";
			}
...
$is_default_values = '[{"text":"Да", "value":"1"},{"text":"Нет", "value":"0"}]';
		...
		$html = '
			...
			<div class="form-group">
				...
				<div>
					<select id="format_id" name="format_id" class="form-control input-md " >
						'.$format_id_options_html.'
						</select>
				</div>
			</div>
...
$is_default_values = '[{"text":"Да", "value":"1"},{"text":"Нет", "value":"0"}]';
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Форматы для {$_REQUEST["book_name"]}".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `book_formats` SET `` = ? WHERE id = ?", [$i, $line[$i]]);
		}
...
$book_id = $_REQUEST['book_id'];
		...
		$sql = "INSERT INTO book_formats (`format_id`, `price`, `is_default`, `book_id`) VALUES (?, ?, ?, ?)";
		if(function_exists("processInsertQuery"))
		{
			$sql = processInsertQuery($sql);
		}

		qi($sql, [$format_id, $price, $is_default, $book_id]);
		$last_id = qInsertId();
	...
	$actions['edit_execute'] = function()
	{
		...
		if(!$skip)
		{
			...
			$set[] = is_null($_REQUEST['format_id'])?"`format_id`=NULL":"`format_id`='".addslashes($_REQUEST['format_id'])."'";
...
$set[] = is_null($_REQUEST['book_id'])?"`book_id`=NULL":"`book_id`='".addslashes($_REQUEST['book_id'])."'";
			...
			{
				$set = implode(", ", $set);
				$sql = "UPDATE book_formats SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM book_formats WHERE id=?", [$id]);
...
$is_default_values = '[{"text":"Да", "value":"1"},{"text":"Нет", "value":"0"}]';
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM book_formats main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM book_formats main_table) temp $srch $filter $where $order";
			if(function_exists("processSelectQuery"))
			{
				$sql = processSelectQuery($sql);
			}
	...
	echo masterRender("Форматы для {$_REQUEST["book_name"]}", $content, 7);
