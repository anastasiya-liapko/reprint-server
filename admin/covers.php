...
$next_order['default_price']='asc';
		...
		<script>
				...
				{
					$(\'.big-icon\').html(\'<i class="fas fa-bookmark"></i>\');
				};
		...
		<div class="content-header">
			<div class="btn-wrap">
				<h2><a href="#" class="back-btn"><span class="fa fa-arrow-circle-left"></span></a> '."Типы переплета".' </h2>
...
<th></th>
			...
			<th>
				   <a href=\'?'.get_query().'&srch-term='.$_REQUEST['srch-term'].'&sort_by=default_price&sort_order='. ($next_order['default_price']) .'\' class=\'sort\' column=\'default_price\' sort_order=\''.$sort_order['default_price'].'\'>Цена по умолчанию'. $sort_icon['default_price'].'</a>
			</th>
					<th></th>
				</tr>
		</thead><tbody>';
...
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=covers' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span></td>", $item, "Название"):"<td><span class='editable ' data-placeholder='' data-inp='text' data-url='engine/ajax.php?action=editable&table=covers' data-pk='{$item['id']}' data-name='name'>".htmlspecialchars($item['name'])."</span></td>")."
".(function_exists("processTD")?processTD("<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=covers' data-pk='{$item['id']}' data-name='default_price'>".htmlspecialchars($item['default_price'])."</span></td>", $item, "Цена по умолчанию"):"<td><span class='editable ' data-placeholder='' data-inp='decimal' data-url='engine/ajax.php?action=editable&table=covers' data-pk='{$item['id']}' data-name='default_price'>".htmlspecialchars($item['default_price'])."</span></td>")."
	...
	$actions['edit'] = function()
	{
		...
		if(isset($id))
		{
			$item = q("SELECT * FROM covers WHERE id=?",[$id]);
			$item = $item[0];
		}
			...
			<form class="form" enctype="multipart/form-data" method="POST">
				<fieldset>'.
		          ...
		          <div class="form-group not-editable">
	               ...
	               <div class="form-group">
	                 <label class="control-label" for="textinput">Цена по умолчанию</label>
	                 <div>
	                   <input id="default_price" name="default_price" type="number" step="0.01" class="form-control input-md " placeholder=""  value="'.htmlspecialchars($item["default_price"]).'">
	                 </div>
	               </div>
	...
	$actions['edit_page'] = function()
	{
		...
		$html = '
			<h1 style="line-height: 30px"> Редактирование <br /><small>'."Типы переплета".' #'.$id.'</small></h1>
			<form class="form" enctype="multipart/form-data" method="POST">
	...
	$actions['reorder'] = function()
	{
		...
		{
			qi("UPDATE `covers` SET `orderby` = ? WHERE id = ?", [$i, $line[$i]]);
		}
	...
	$actions['create_execute'] = function()
	{
		...
		$name = $_REQUEST['name'];
$default_price = $_REQUEST['default_price'];

		$sql = "INSERT INTO covers (`name`, `default_price`) VALUES (?, ?)";
		if(function_exists("processInsertQuery"))
		{
			$sql = processInsertQuery($sql);
		}

		qi($sql, [$name, $default_price]);
		$last_id = qInsertId();
...
$set[] = is_null($_REQUEST['default_price'])?"`default_price`=NULL":"`default_price`='".addslashes($_REQUEST['default_price'])."'";

			if(count($set)>0)
			{
				$set = implode(", ", $set);
				$sql = "UPDATE covers SET $set WHERE id=?";
	...
	$actions['delete'] = function()
	{
		...
		try
		{
			qi("DELETE FROM covers WHERE id=?", [$id]);
	...
	function get_data($force_kill_pagination=false)
	{
		...
		if($pagination == 1)
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT  main_table.*  FROM covers main_table) temp $srch $filter $where $order LIMIT :start, :limit";
		...
		{
			$sql = "SELECT SQL_CALC_FOUND_ROWS * FROM (SELECT main_table.*  FROM covers main_table) temp $srch $filter $where $order";
			if(function_exists("processSelectQuery"))
			{
				$sql = processSelectQuery($sql);
			}
	...
	echo masterRender("Типы переплета", $content, 1);
